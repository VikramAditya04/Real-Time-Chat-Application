      ...
      <div className="mx-auto flex min-h-[calc(100vh-2.5rem)] w-full max-w-7xl overflow-hidden rounded-[28px] border border-white/10 bg-white/5 shadow-[0_30px_120px_rgba(0,0,0,0.55)] backdrop-blur-2xl">
        <div className="hidden flex-1 flex-col justify-between border-r border-white/10 bg-[radial-gradient(circle_at_top_left,rgba(56,189,248,0.18),transparent_32%),linear-gradient(180deg,rgba(10,16,30,0.95),rgba(6,10,20,0.86))] p-8 lg:flex">
          <div>
            ...
            <p className="mt-4 max-w-md text-sm leading-7 text-slate-300">
              Chats, photos, and reactions stay in sync-sign up on the right to continue.
            </p>
          </div>
        ...
        <div className="flex flex-1 items-center justify-center p-5 sm:p-8">
          <div className="w-full max-w-md">
            ...
            <div className="glass-panel rounded-4xl p-5 sm:p-7">
...
export default function SignUpPage({ onSwitchAuth, onGoHome, onSignupSuccess }) {
  ...
  return (
    <AuthShell
      ...
      subtitle="Create a Chatzy account to continue."
    ...
    >
      <form className="space-y-4" onSubmit={handleSubmit}>
        <div className="space-y-2">
          <label htmlFor="signup-name" className="text-xs uppercase tracking-[0.3em] text-slate-400">Full name</label>
          <input id="signup-name" required autoComplete="name" value={form.fullName} onChange={(e) => setForm({ ...form, fullName: e.target.value })} className={fieldBase} type="text" placeholder="Enter your name" />
        </div>
        <div className="space-y-2">
          <label htmlFor="signup-email" className="text-xs uppercase tracking-[0.3em] text-slate-400">Email address</label>
          <input id="signup-email" required autoComplete="email" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} className={fieldBase} type="email" placeholder="Enter your email" />
        </div>
        <div className="space-y-2">
          <label htmlFor="signup-password" className="text-xs uppercase tracking-[0.3em] text-slate-400">Password</label>
          <input id="signup-password" required minLength={6} autoComplete="new-password" value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })} className={fieldBase} type="password" placeholder="At least 6 characters" />
        </div>
        {error && <p role="alert" className="rounded-xl border border-rose-400/20 bg-rose-500/10 px-3 py-2 text-sm text-rose-200">{error}</p>}
        <button disabled={isSigningUp} type="submit" className="btn-primary w-full rounded-2xl px-6 py-4 text-base font-semibold text-white transition hover:brightness-110 disabled:cursor-not-allowed disabled:opacity-60">
